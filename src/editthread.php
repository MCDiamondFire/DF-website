...
<html>
  <head>
    <title>
      <?php $pageTitle = getLanguageString("pageEditThreadTitle");
      echo $pageTitle; ?></title>
    </title>
    <?php require_once $root . 'assets/php/requires/head.php'; ?>
    <link href="<?php echo CSS . 'editThread.css'; ?>" rel="stylesheet">
  </head>
  ...
  <body>
    ...
    <div id="content">
      ...
      <div id="fullWidthPanelWrapper">
        <div id="panelHeading">
          <input maxlength="32" type="text" id="threadName" placeholder="<?php echo getLanguageString("threadName"); ?>" value="<?php echo $row['name']; ?>">
          <input maxlength="32" type="text" id="threadDescription" placeholder="<?php echo getLanguageString("threadDescription"); ?>" value="<?php echo $row['description']; ?>">
        </div>
        <div id="panelContent">
  ...
  <script>
    ...
    function save(button) {
      ...
      $.ajax({
        type: "POST",
        url: "<?php echo $page; ?>assets/php/core/actions/savethread.php",
        data: {
          name: $('#threadName').val(),
